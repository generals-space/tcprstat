...
static int lock_stats(void), unlock_stats(void);
...
/**
 * 
 * caller: process-packet.c -> process_ip()
 */
int outbound(struct timeval tv, struct in_addr laddr, struct in_addr raddr, uint16_t lport, uint16_t rport)
{
    struct timeval start;
    // newstat 将会放到 stats 数组中, 所以这里的 unsigned long 其实应该是指针地址吧???
    ...
    {
        newstat = (tv.tv_sec - start.tv_sec) * 1000000 + (tv.tv_usec - start.tv_usec);
...
static void sort_results(struct stats_results *results) {
    qsort(results->stats, results->statscount, sizeof(unsigned long), compare_stats);
    results->sorted = 1;
}
...
unsigned long stats_avg(struct stats_results *results, int percentile) {
    ...
    if (!results->statscount) return 0;

    if (percentile == 0 || percentile == 100)
        n = results->statscount;
    else {
        if (!results->sorted) sort_results(results);
